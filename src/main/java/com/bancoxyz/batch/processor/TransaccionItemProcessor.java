...
public class TransaccionItemProcessor implements ItemProcessor<TransaccionCsv, Transaccion> {
    ...
    /** Identificadores ya vistos: un id repetido significa que el archivo trae la misma
     *  transaccion dos veces (por ejemplo, dos cortes legacy concatenados). */
    private final DetectorDeDuplicados identificadores = new DetectorDeDuplicados();
    /** Claves de contenido, para detectar transacciones sospechosamente iguales. */
    private final DetectorDeDuplicados contenidos = new DetectorDeDuplicados();
    ...
    public Transaccion process(TransaccionCsv fila) {
        ...
        if (identificadores.esDuplicado(String.valueOf(idOrigen), linea)) {
        ...
        if (contenidos.esDuplicado(claveContenido, linea)) {
            anomalia = true;
