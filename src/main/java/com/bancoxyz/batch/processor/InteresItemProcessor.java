...
public class InteresItemProcessor implements ItemProcessor<InteresCsv, CuentaInteres> {
    ...
    /** Fila completa repetida: es una copia y se descarta. */
    private final DetectorDeDuplicados filasIdenticas = new DetectorDeDuplicados();
    /** Cuenta repetida con datos distintos: se procesa, pero queda marcada. */
    private final DetectorDeDuplicados cuentas = new DetectorDeDuplicados();
    /** Mismo titular con exactamente los mismos datos en otra cuenta. */
    private final DetectorDeDuplicados titulares = new DetectorDeDuplicados();
    ...
    public CuentaInteres process(InteresCsv fila) {
        ...
        if (filasIdenticas.esDuplicado(crudo, linea)) {
        ...
        if (cuentas.esDuplicado(String.valueOf(cuentaId), linea)) {
            anomalia = true;
        ...
        String claveTitular = nombre.toLowerCase() + "|" + saldo.stripTrailingZeros().toPlainString()
                + "|" + edad + "|" + tipo;
        if (titulares.esDuplicado(claveTitular, linea)) {
            anomalia = true;
