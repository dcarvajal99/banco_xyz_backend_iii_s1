...
public class MovimientoAnualItemProcessor implements ItemProcessor<MovimientoAnualCsv, MovimientoAnual> {
    ...
    /** Movimientos identicos ya vistos en la corrida: se marcan, no se descartan. */
    private final DetectorDeDuplicados movimientos = new DetectorDeDuplicados();

    public MovimientoAnualItemProcessor(@Value("#{stepExecution.jobExecutionId}") Long jobExecutionId) {
    ...
    public MovimientoAnual process(MovimientoAnualCsv fila) {
        ...
        if (movimientos.esDuplicado(clave, fila.getNumeroLinea())) {
            anomalia = true;
